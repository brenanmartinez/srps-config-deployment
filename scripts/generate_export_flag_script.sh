...
-- Enable export flag for transaction types (referenced by exported transactions)
UPDATE tt
SET tt.C_EXPORT = 1
FROM T_PSO_TRANSACTION_TYPE tt
INNER JOIN T_PSO_TRANSACTION_DEFINITION td ON tt.ID = td.C_TRANSACTION_TYPE
WHERE td.C_EXPORT = 1
AND td.C_TRANSACTION_TYPE IS NOT NULL;
